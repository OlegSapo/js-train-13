...
function manipulateUrl(url) {
  ...
  // console.log(urlTemp);
  urlTemp.protocol = "https:";
  urlTemp.host = "newhost.com";
  urlTemp.searchParams.append("newParam", "newValue");
  if (urlTemp.searchParams.has("oldParam")) {
    urlTemp.searchParams.delete("oldParam");
  }
  return urlTemp;
}
...
function searchParamsURL(url) {
  ...
  let URLSearchParams = urlNew.searchParams;
  let params = new Map();
...
function manipulateSearchParams(paramsObj, newUrl) {
  ...
  let keysNewURL = Object.keys(paramsObj);
...
function getUrlData(url) {
  ...
  let urlObj = new URL(url);
  let infoURL = {};
  infoURL["origin"] = urlObj.origin;
  infoURL["hostname"] = urlObj.hostname;
  infoURL["port"] = urlObj.port;
  infoURL["username"] = urlObj.username;
  infoURL["password"] = urlObj.password;
  return infoURL;
}
...
function sortUrlParams(url) {
  ...
  // Повертаємо новий URL як рядок.
  let urlObj = new URL(url);
  let arrParams = Array.from(urlObj.searchParams.entries());
  arrParams.sort();
...
function getURLValues(url) {
  ...
  let arrKeys = Array.from(URLSearchParams.keys());
  let arrSearch = [];
  for (let key of arrKeys) {
    // console.log(key);
    arrSearch.push(key);
    arrSearch.push(urlObj.searchParams.getAll(key));
  }
  // console.log(arrKeys);
...
function getUrlKeys(url) {
  ...
  let urlObj = new URL(url);
  let arrKeys = Array.from(urlObj.searchParams.keys());
  return arrKeys;
}
